...
The fix was a policy, not a prompt. From that day the main session, the "brain", writes every spec, plan, tasks file and contract itself. Sub-agents receive a finished design and implement it; open questions go into the spec's Clarifications section, never into code. The Opus re-implementation that followed shipped with zero contract deviations and 26 gap-fill notes, went through the reviewer gate (two real findings, fixed), and merged.
