---
name: medium-variant
description: Produce the Medium variant (full-length, clean Markdown article — a near-passthrough) of an approved canonical piece for manual copy-paste. Use when the owner asks for a Medium version, Medium article, or Medium variant of a piece.
---

# Medium variant

Reformat an approved canonical into a clean Medium article. This is a single reformatting pass —
...
- `content/platforms/medium.md` — the medium playbook: hard format constraints, structure, avoid-list,
...
4. Read `content/platforms/medium.md` (the playbook). Produce the variant per the GROUNDING block,
...
5. Write it to `pieces/<slug>/variants/medium.md` (`mkdir -p pieces/<slug>/variants`). The file
   contains the formatted variant ONLY — no preamble, no notes, no commentary before or after
   it.
6. **Commit:** `git add pieces/<slug>/variants && git commit -m "variant(<slug>): medium"`.
...
## Platform brief (ported verbatim from app/variants/format.py _PLATFORM_BRIEFS["medium"])
